...
export function WhatsAppFab() {
  ...
  return (
    <div className="pointer-events-none fixed bottom-6 right-4 z-40 flex flex-col items-end gap-4 sm:right-6">
      <a
        href={waGeneral()}
        target="_blank"
        rel="noopener"
        ...
        data-cursor="link"
        className={cn(
          "breathe group pointer-events-auto grid h-14 w-14 place-items-center rounded-full border border-border backdrop-blur-[22px] transition-all duration-[var(--dur-standard)] ease-[var(--ease-enter)]",
          shown ? "translate-y-0 opacity-100" : "pointer-events-none translate-y-6 opacity-0",
        )}
        style={{
          backgroundImage:
            "linear-gradient(135deg, color-mix(in oklab, var(--fleece) 12%, transparent), color-mix(in oklab, var(--fleece) 3%, transparent))",
          boxShadow: "0 24px 60px -20px color-mix(in oklab, var(--ink) 80%, transparent)",
        }}
      >
        <svg viewBox="0 0 24 24" className="h-6 w-6 fill-fleece" aria-hidden>
        ...
        <span
          className="absolute right-16 whitespace-nowrap rounded-full border border-border px-3 py-1.5 font-data text-2xs text-fleece opacity-0 transition-opacity duration-[var(--dur-micro)] group-hover:opacity-100"
